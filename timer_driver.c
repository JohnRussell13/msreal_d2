...
static void setup_and_start_timer(unsigned int milliseconds)
{
	...
	unsigned int zero = 0;
	unsigned int data = 0;
	timer_load = zero - milliseconds*100000*1000;
	...
	data = ioread32(tp->base_addr + XIL_AXI_TIMER_TCSR_OFFSET);
	iowrite32(data | XIL_AXI_TIMER_CSR_ENABLE_TMR_MASK | XIL_AXI_TIMER_CSR_CASC_MASK,
			tp->base_addr + XIL_AXI_TIMER_TCSR_OFFSET);
...
ssize_t timer_read(struct file *pfile, char __user *buffer, size_t length, loff_t *offset) 
{
	...
	printk(KERN_INFO "Succesfully read timer. Time left: %d:%d:%d:%d\n",day,hour,minute,second);
	return 0;
}
